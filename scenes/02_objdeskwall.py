...
DROP_CENTER_OFFSET = (0.0, 0.0, 0.15)
...
# 최종 dataset 저장 위치
DATASET_ROOT = "/isaac-sim/corl2025/dataset/02/"
...
def filter_objects_in_bounds(stage, object_paths):
    in_bounds = []
    for path in object_paths:
        prim = stage.GetPrimAtPath(path)
        if not prim.IsValid():
            print(f"[Save] skip invalid prim {path}")
            continue
        ...
        if (
            ...
            in_bounds.append(path)
        else:
            print(
                f"[Save] exclude (out of bounds): {path} "
                f"x={position[0]:.3f}, y={position[1]:.3f} "
                f"x[{SAVE_X_MIN:.3f}, {SAVE_X_MAX:.3f}] "
                f"y[{SAVE_Y_MIN:.3f}, {SAVE_Y_MAX:.3f}]"
            )

    print(f"[Save] {len(in_bounds)}/{len(object_paths)} objects within save bounds")
    return in_bounds
...
async def main():
    ...
    save_paths = filter_objects_in_bounds(stage, object_paths)
    if not save_paths:
        cleanup_dataset_scene_dir(scene_dir)
        print("\n[SKIP SAVE]")
        print("Scene was not saved because no objects are within bounds.")
        return

    save_objects_json(stage, save_paths, spawned_assets, str(json_save_path))
    save_objects_usda(stage, save_paths, str(usda_save_path))
