...
RESET_FRAMES = 120
LINEAR_MOVE_DISTANCE_M = 0.15
LINEAR_MOVE_STEPS = 50
...
def _default_output_path(args: argparse.Namespace, branch: str) -> Path:
	idx = 0
	if args.scene_num is not None:
		idx = int(args.scene_num)
	dataset_name = "dataset"
	output_group = f"heuristic_{branch}"
	if args.jitter:
		output_group += "_jit"
...
def _parse_args() -> argparse.Namespace:
	parser = argparse.ArgumentParser(
		description="Run heuristic baseline experiment with linear initial moves."
	)
...
def _pose_to_matrix44(pos: np.ndarray, quat_wxyz: np.ndarray) -> np.ndarray:
	"""Create a 4x4 transformation matrix from position and quaternion (wxyz)."""
...
def _get_prim_pose_wxyz(stage: Usd.Stage, prim_path: str) -> tuple[np.ndarray, np.ndarray]:
	"""Get prim pose as (position, quaternion_wxyz)."""
...
def _set_prim_pose_wxyz(stage: Usd.Stage, prim_path: str, pos: np.ndarray, quat_wxyz: np.ndarray) -> None:
	"""Set prim pose from position and quaternion (wxyz)."""
	prim = stage.GetPrimAtPath(prim_path)
	if not prim.IsValid():
		return
...
def _open_gripper(robot: SingleArticulation, controller, world: World, frames: int | None = None) -> None:
	if frames is None:
...
def _close_gripper(robot: SingleArticulation, controller, world: World, grip_value: float | None = None, frames: int | None = None) -> None:
...
def _build_curobo_world(stage: Usd.Stage, robot_prim_path: str, ignore_paths: list[str]) -> WorldConfig:
	...
	return _merge_world_configs(stage_obstacles, env_collision_world).get_collision_check_world()


def _create_motion_gen(stage: Usd.Stage, ignore_paths: list[str]) -> MotionGen:
	setup_curobo_logger("error")
	tensor_args = TensorDeviceType()
	robot_cfg = load_yaml(join_path(get_robot_configs_path(), "franka.yml"))["robot_cfg"]
	world_cfg = _build_curobo_world(stage, ARGS.robot_prim_path, ignore_paths)
...
def _get_gripper_z_direction(ik_solver: KinematicsSolver) -> np.ndarray:
	"""Get the Z-axis direction of the gripper (end-effector) in world frame."""
	ee_pos, ee_quat = _get_end_effector_pose_wxyz(ik_solver)
	mat = _pose_to_matrix44(ee_pos, ee_quat)
	return mat[:3, 2]


def _move_linear_approach(
	world: World,
	robot: SingleArticulation,
	ik_solver: KinematicsSolver,
	controller,
	...
	"""Linear move along negative gripper direction (15cm). Returns (success, reason, intermediate_position)."""
	try:
		ee_pos_start, ee_quat_start = _get_end_effector_pose_wxyz(ik_solver)
		gripper_z_dir = _get_gripper_z_direction(ik_solver)
		move_direction = -gripper_z_dir
		move_direction = move_direction / np.linalg.norm(move_direction)
		
		waypoints = []
		for step in range(LINEAR_MOVE_STEPS + 1):
			alpha = step / LINEAR_MOVE_STEPS
			wp = ee_pos_start + alpha * LINEAR_MOVE_DISTANCE_M * move_direction
			waypoints.append(wp)
		
		intermediate_pos = waypoints[-1]  # Target intermediate position
		
		arm_joint_ids = [i for i, name in enumerate(robot.dof_names) if "finger" not in name.lower() and "gripper" not in name.lower()]
		for waypoint in waypoints[1:]:
			ik_action, ik_ok = ik_solver.compute_inverse_kinematics(
				target_position=np.asarray(waypoint, dtype=np.float64),
				target_orientation=np.asarray(ee_quat_start, dtype=np.float64),
			)
			if not ik_ok or ik_action is None or ik_action.joint_positions is None:
				return False, "IK failed during linear approach move", None
			current_q = robot.get_joint_positions()
			if current_q is None:
				return False, "Failed to read robot joint positions", None
			goal_q = np.asarray(current_q, dtype=np.float64).copy()
			ik_q = np.asarray(ik_action.joint_positions, dtype=np.float64).flatten()
			if getattr(ik_action, "joint_indices", None) is not None:
				for local_i, joint_i in enumerate(np.asarray(ik_action.joint_indices, dtype=np.int64)):
					if 0 <= int(joint_i) < goal_q.shape[0]:
						goal_q[int(joint_i)] = ik_q[local_i]
			elif ik_q.shape[0] == len(arm_joint_ids):
				for local_i, joint_i in enumerate(arm_joint_ids):
					goal_q[joint_i] = ik_q[local_i]
			else:
				goal_q[:ik_q.shape[0]] = ik_q
			controller.apply_action(ArticulationAction(joint_positions=goal_q))
			for _ in range(ARGS.steps_per_waypoint):
				world.step(render=not ARGS.headless)
				ee_pos_step, ee_quat_step = _get_end_effector_pose_wxyz(ik_solver)
				_update_target_transform(stage, target_prim_path, ee_pos_step, ee_quat_step, obj_in_ee)
		return True, None, intermediate_pos
	except Exception as exc:
		return False, f"Linear approach move failed: {exc}", None


def _move_linear_vertical(
	world: World,
	robot: SingleArticulation,
	ik_solver: KinematicsSolver,
	controller,
	...
	"""Linear move vertically upward (15cm). Returns (success, reason, intermediate_position)."""
	try:
		ee_pos_start, ee_quat_start = _get_end_effector_pose_wxyz(ik_solver)
		move_direction = np.array([0.0, 0.0, 1.0], dtype=np.float64)
		...
		for waypoint in waypoints[1:]:
			...
			if not ik_ok or ik_action is None or ik_action.joint_positions is None:
				return False, "IK failed during linear vertical move", None
			current_q = robot.get_joint_positions()
			if current_q is None:
				return False, "Failed to read robot joint positions", None
			goal_q = np.asarray(current_q, dtype=np.float64).copy()
		...
		return False, f"Linear vertical move failed: {exc}", None


def _run_heuristic_branch(
	branch_name: str,
	motion_gen: MotionGen,
	world: World,
	robot: SingleArticulation,
	...
	"""Run a single heuristic branch (approach or vertical)."""
	sim_result = []
	open_space_destinations = _open_space_destinations()
	
	for dest_idx, open_space_destination in enumerate(open_space_destinations):
		msg = f"[{branch_name}] Destination {dest_idx + 1}/{len(open_space_destinations)} target={open_space_destination.tolist()}"
		carb.log_info(msg)
		...
		if not target_prim.IsValid():
			carb.log_warn(f"[{branch_name}] Target prim not found for destination {dest_idx + 1}: {target_prim_path}")
			sim_result.append({
		...
		if not grasp_ok:
			carb.log_warn(f"[{branch_name}] Grasp failed for destination {dest_idx + 1}: {grasp_reason}")
			trial_objects_after = _snapshot_objects(stage, all_prim_paths)
			sim_result.append({
				"destination": open_space_destination.tolist(),
				"ik_success": False,
		...
		_close_gripper(robot, controller, world)
		...
		if branch_name == "approach":
			linear_ok, linear_reason, intermediate_target = _move_linear_approach(
				world=world,
				robot=robot,
				ik_solver=ik_solver,
				controller=controller,
			...
			linear_ok, linear_reason, intermediate_target = _move_linear_vertical(
				world=world,
				robot=robot,
				ik_solver=ik_solver,
				controller=controller,
		...
		if not linear_ok:
			carb.log_warn(f"[{branch_name}] Linear move failed for destination {dest_idx + 1}: {linear_reason}")
			trial_objects_after = _snapshot_objects(stage, all_prim_paths)
			sim_result.append({
				"destination": open_space_destination.tolist(),
				"ik_success": False,
				"move_success": False,
				"failure_reason": linear_reason,
				"objects_after": trial_objects_after,
			})
			continue
		
		# Validate intermediate position before proceeding to final destination
		ee_pos_after_linear, _ = _get_end_effector_pose_wxyz(ik_solver)
		intermediate_error = float(np.linalg.norm(ee_pos_after_linear - intermediate_target))
		if intermediate_error > MOVE_SUCCESS_POS_TOL_M:
			carb.log_warn(f"[{branch_name}] Failed to reach intermediate position for destination {dest_idx + 1}, error={intermediate_error:.4f}m")
			trial_objects_after = _snapshot_objects(stage, all_prim_paths)
			sim_result.append({
				"destination": open_space_destination.tolist(),
				"ik_success": False,
				"move_success": False,
				"failure_reason": f"Intermediate position error {intermediate_error:.4f}m exceeds tolerance",
				"objects_after": trial_objects_after,
			})
			continue
		...
		trial_objects_after = _snapshot_objects(stage, all_prim_paths)
		if not open_ok:
			carb.log_warn(f"[{branch_name}] Move failed for destination {dest_idx + 1}: {open_reason}")
			sim_result.append({
				"destination": open_space_destination.tolist(),
		...
		if trial_move_success:
			carb.log_info(f"[{branch_name}] Success for destination {dest_idx + 1}, pos_error={pos_error:.4f}")
		else:
			carb.log_warn(f"[{branch_name}] Reached destination {dest_idx + 1} but move_success false, pos_error={pos_error:.4f}")
...
def main() -> None:
	...
	all_prim_paths = [obj_data.get("prim_path") for obj_data in all_objects_data]
	...
	carb.log_info("[Heuristic] Initializing CuRobo motion generator...")
	motion_gen = _create_motion_gen(stage, ignore_paths_common)
	carb.log_info("[Heuristic] CuRobo initialized.")

	for branch_name in ["approach", "vertical"]:
		carb.log_info(f"[Heuristic] Starting {branch_name} branch...")
		
		sim_result = _run_heuristic_branch(
			...
			objects_usda=objects_usda,
			object_root=object_root,
			ignore_paths_grasp=ignore_paths_grasp,
			ignore_paths_with_target=ignore_paths_with_target,
		)
		...
		if ARGS.output_json:
			...
			output_path = _default_output_path(ARGS, branch_name)
		...
		print(f"[DONE] Saved {branch_name} experiment result: {output_path}", flush=True)
...
if __name__ == "__main__":
	try:
		...
		print(f"[ERROR] experiment_heu failed: {exc}", flush=True)
		traceback.print_exc()
		raise
	finally:
