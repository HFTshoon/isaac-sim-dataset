...
def _default_output_path(args: argparse.Namespace) -> Path:
	...
	output_group = "rrt_withobj" if args.include_target else "rrt"
	if args.jitter:
		output_group += "_jit"
...
from isaacsim.robot_motion.motion_generation import ArticulationTrajectory, interface_config_loader
...
def _get_prim_world_aabb(stage: Usd.Stage, prim_path: str) -> tuple[np.ndarray, np.ndarray]:
	prim = stage.GetPrimAtPath(prim_path)
	if not prim.IsValid():
...
def _aabbs_overlap(a_min: np.ndarray, a_max: np.ndarray, b_min: np.ndarray, b_max: np.ndarray) -> bool:
...
def _iter_object_collision_geom_aabbs(stage: Usd.Stage, object_prim_path: str) -> list[tuple[np.ndarray, np.ndarray]]:
	"""Return AABBs of collision-enabled geometry under object prim for tighter RRT approximation."""
	root_prim = stage.GetPrimAtPath(object_prim_path)
	if not root_prim.IsValid():
		return []
	aabbs: list[tuple[np.ndarray, np.ndarray]] = []
	for prim in Usd.PrimRange(root_prim):
		if not _has_collision_enabled_ancestor(prim, object_prim_path):
			continue
		if not (prim.IsA(UsdGeom.Mesh) or prim.IsA(UsdGeom.Cube) or prim.IsA(UsdGeom.Sphere) or prim.IsA(UsdGeom.Cylinder) or prim.IsA(UsdGeom.Capsule)):
			continue
		try:
			bb_min, bb_max = _get_prim_world_aabb(stage, str(prim.GetPath()))
		except Exception:
			continue
		aabbs.append((bb_min, bb_max))
	if len(aabbs) == 0:
		try:
			aabbs.append(_get_prim_world_aabb(stage, object_prim_path))
		except Exception:
			pass
	return aabbs


def _create_rrt_planner(
	world: World,
	robot: SingleArticulation,
	stage: Usd.Stage,
	object_prim_paths: list[str],
) -> tuple[RRT, LulaCSpaceTrajectoryGenerator, PathPlannerVisualizer, dict[str, list[VisualCuboid]], dict[str, bool]]:
	...
	planner_root = "/World/PlannerObstacles"
	_ensure_xform(stage, planner_root)
	obstacle_cuboids: dict[str, list[VisualCuboid]] = {}
	obstacle_enabled_state: dict[str, bool] = {}
	obstacle_index = 0
	for object_path in object_prim_paths:
		aabbs = _iter_object_collision_geom_aabbs(stage, object_path)
		if len(aabbs) == 0:
			carb.log_warn(f"Failed to register RRT obstacle for {object_path}: no collision geometry AABB")
			continue
		obstacle_cuboids[object_path] = []
		for bb_min, bb_max in aabbs:
			scale = np.maximum(
				(bb_max - bb_min) + np.array([2.0 * 0.005, 2.0 * 0.005, 2.0 * 0.005], dtype=np.float64),
				np.array([0.015, 0.015, 0.015], dtype=np.float64),
			)
			center = 0.5 * (bb_min + bb_max)
			cuboid = world.scene.add(
				VisualCuboid(
					prim_path=f"{planner_root}/object_obstacle_{obstacle_index}",
					name=f"object_obstacle_{obstacle_index}",
					position=center,
			...
			obstacle_cuboids[object_path].append(cuboid)
			obstacle_index += 1
		obstacle_enabled_state[object_path] = True
	rrt.update_world()
	return rrt, traj_gen, planner_visualizer, obstacle_cuboids, obstacle_enabled_state


def _configure_rrt_open_obstacles(
	rrt: RRT,
	obstacle_cuboids: dict[str, list[VisualCuboid]],
	obstacle_enabled_state: dict[str, bool],
	target_prim_path: str,
) -> None:
	for object_path, obstacle_list in obstacle_cuboids.items():
		desired_enabled = object_path != target_prim_path
		...
		try:
			for obstacle in obstacle_list:
				if desired_enabled:
					rrt.enable_obstacle(obstacle)
				else:
					rrt.disable_obstacle(obstacle)
			obstacle_enabled_state[object_path] = desired_enabled
		except Exception as exc:
			carb.log_warn(f"Failed to toggle RRT obstacle {object_path}: {exc}")
	rrt.update_world()


def _attached_target_collision_path(
	stage: Usd.Stage,
	target_prim_path: str,
	other_prim_paths: list[str],
) -> str | None:
	target_aabbs = _iter_object_collision_geom_aabbs(stage, target_prim_path)
	if len(target_aabbs) == 0:
		return None
	for prim_path in other_prim_paths:
		other_aabbs = _iter_object_collision_geom_aabbs(stage, prim_path)
		if len(other_aabbs) == 0:
			continue
		for target_bb_min, target_bb_max in target_aabbs:
			for other_bb_min, other_bb_max in other_aabbs:
				if _aabbs_overlap(target_bb_min, target_bb_max, other_bb_min, other_bb_max):
					return prim_path
	return None


def _move_pose_with_rrt(
	world: World,
	rrt: RRT,
	traj_gen: LulaCSpaceTrajectoryGenerator,
	planner_visualizer: PathPlannerVisualizer,
	robot: SingleArticulation,
	...
	non_target_prim_paths: list[str] | None = None,
) -> tuple[bool, np.ndarray, str | None]:
	_configure_rrt_open_obstacles(rrt, obstacle_cuboids, obstacle_enabled_state, target_prim_path)
	if include_target and stage is not None and non_target_prim_paths is not None:
		collision_path = _attached_target_collision_path(stage, target_prim_path, non_target_prim_paths)
		if collision_path is not None:
			current_ee_pos, _ = ik_solver.compute_end_effector_pose(position_only=True)
			return False, np.array(current_ee_pos, dtype=np.float64), f"start=attached target collides with {collision_path}"
	...
	rrt.set_max_iterations(max(100, int(ARGS.rrt_max_iterations)))
	active_joints = planner_visualizer.get_active_joints_subset()
	start_pos = active_joints.get_joint_positions()
	if start_pos is None:
		current_ee_pos, _ = ik_solver.compute_end_effector_pose(position_only=True)
		return False, np.array(current_ee_pos, dtype=np.float64), "Failed to read active joint positions for RRT"
	rrt_plan = rrt.compute_path(start_pos, np.array([]))
	if rrt_plan is None or len(rrt_plan) <= 1:
		current_ee_pos, _ = ik_solver.compute_end_effector_pose(position_only=True)
		return False, np.array(current_ee_pos, dtype=np.float64), "RRTConnect failed to find a path"
	...
	actions = ArticulationTrajectory(robot, trajectory, 1.0 / 60.0).get_action_sequence()
	if len(actions) == 0:
		current_ee_pos, _ = ik_solver.compute_end_effector_pose(position_only=True)
		return False, np.array(current_ee_pos, dtype=np.float64), "RRTConnect produced an empty action sequence"
	for action in actions:
		controller.apply_action(action)
		for _ in range(max(1, ARGS.steps_per_waypoint)):
			world.step(render=not ARGS.headless)
			if stage is not None and obj_in_ee is not None:
				...
				if include_target and non_target_prim_paths is not None:
					collision_path = _attached_target_collision_path(stage, target_prim_path, non_target_prim_paths)
					if collision_path is not None:
						return False, ee_pos_step, f"attached target collides with {collision_path}"
	final_ee_pos, _ = ik_solver.compute_end_effector_pose(position_only=True)
	return True, np.array(final_ee_pos, dtype=np.float64), None
...
def main() -> None:
	...
	non_target_prim_paths = [path for path in all_prim_paths if path != target_prim_path]
	rrt, traj_gen, planner_visualizer, obstacle_cuboids, obstacle_enabled_state = _create_rrt_planner(
		world,
		robot,
		stage,
		all_prim_paths,
	...
	for dest_idx, open_space_destination in enumerate(open_space_destinations):
		...
		open_ok, final_ee_pos, open_reason = _move_pose_with_rrt(
			world=world,
			rrt=rrt,
			traj_gen=traj_gen,
			...
			obstacle_enabled_state=obstacle_enabled_state,
			stage=stage,
			obj_in_ee=obj_in_ee,
			include_target=ARGS.include_target,
			non_target_prim_paths=non_target_prim_paths,
		)
...
if __name__ == "__main__":
	try:
		...
		print(f"[ERROR] experiment_rrt failed: {exc}", flush=True)
		traceback.print_exc()
		raise
	finally:
